...
const series = require('./resources/series')
...
/*
  ...
  Create a get all, get one, create, update, and delete route for the `character` resource.
  ...
  Each character must have a name and series_id. The series_id must match an existing series. Dynamically generate the id using `uuid()` on creation.
*/
...
/*
  ...
  Implement a PATCH route for both resources. Your existing PUT route should now require all fields to be present per resource.
*/

/*
  STRETCH 2:

  You likely have some code that is very nearly the same with only a few differences. Refactor so that, for example, your GET routes use the same code regardless of the resource.
*/
